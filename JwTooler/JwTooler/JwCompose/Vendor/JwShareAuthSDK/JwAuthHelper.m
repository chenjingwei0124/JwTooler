...
@property (nonatomic, copy) AuthiOSBlock didAuthiOSBlock;
...
+ (void)signinAppleWithcompletion:(AuthiOSBlock)completion API_AVAILABLE(ios(13.0)){
    ...
    JwAuthHelper *authHelper = [JwAuthHelper singleAuth];
    authHelper.didAuthiOSBlock = completion;
...
- (void)authorizationController:(ASAuthorizationController *)controller didCompleteWithAuthorization:(ASAuthorization *)authorization API_AVAILABLE(ios(13.0)){
    ...
    if([authorization.credential isKindOfClass:[ASAuthorizationAppleIDCredential class]]){
        ...
        if (self.didAuthiOSBlock) {
            self.didAuthiOSBlock(YES, userID, nil, authorizationCode, identityToken);
        }
        ...
        if (self.didAuthiOSBlock) {
            self.didAuthiOSBlock(NO, userIdentifier, password, nil, nil);
        }
    }
}
...
- (void)authorizationController:(ASAuthorizationController *)controller didCompleteWithError:(NSError *)error API_AVAILABLE(ios(13.0)){
    ...
    DLog(@"%@", errorMsg);
    if (self.didAuthiOSBlock) {
        self.didAuthiOSBlock(NO, errorMsg, nil, nil, nil);
    }
}
